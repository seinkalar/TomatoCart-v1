...
*/
  ...
  //articles images
      ...
      if ($error === false) {
      	if (is_numeric($id)) {
      		$Qdelete = $osC_Database->query('delete from :table_articles_to_stores where articles_id = :articles_id');
      		$Qdelete->bindTable(':table_articles_to_stores', TABLE_ARTICLES_TO_STORES);
      		$Qdelete->bindInt(':articles_id', $articles_id);
      		$Qdelete->execute();
      	...
      	if ( count($data['stores_ids']) > 0 ) {
      		foreach ($data['stores_ids'] as $stores_id) {
      			$Qinsert = $osC_Database->query('insert into :table_articles_to_stores (articles_id, stores_id) values (:articles_id, :stores_id)');
      			$Qinsert->bindTable(':table_articles_to_stores', TABLE_ARTICLES_TO_STORES);
      			$Qinsert->bindInt(':articles_id', $articles_id);
    ...
    /**
     * Get the stores linked to currect article
     *
     * @acess public
     * @param int
     * @return mixed
     */
    function getStores($articles_id) {
    	global $osC_Database;
    
    	$Qstores = $osC_Database->query('select stores_id from :table_articles_to_stores where articles_id = :articles_id');
    	$Qstores->bindTable(':table_articles_to_stores', TABLE_ARTICLES_TO_STORES);
    	$Qstores->bindInt(':articles_id', $articles_id);
    	$Qstores->execute();
