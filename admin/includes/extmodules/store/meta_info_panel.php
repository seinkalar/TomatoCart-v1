...
?>
Toc.store.MetaInfoPanel = function (config) {
  config = config || {};
  
  config.title = '<?php echo $osC_Language->get('section_meta_info'); ?>';
  config.border = false;
  config.layout = 'fit';
  
  config.items = this.buildForm();

  Toc.store.MetaInfoPanel.superclass.constructor.call(this, config);
};

Ext.extend(Toc.store.MetaInfoPanel, Ext.Panel, {
  buildForm: function () {
