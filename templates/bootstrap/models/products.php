...
function get_new_products() {
    ...
    $Qnewproducts->bindTable(':table_products_frontpage', TABLE_PRODUCTS_FRONTPAGE);
    $Qnewproducts->bindTable(':table_specials', TABLE_SPECIALS);
	...
	if (MODULE_CONTENT_NEW_PRODUCTS_CACHE > 0) {
        $Qnewproducts->setCache('new_products-bootstrap-' . $osC_Language->getCode() . '-' . $osC_Currencies->getCode() . '-' . $current_category_id, MODULE_CONTENT_NEW_PRODUCTS_CACHE);
    }
    ...
    if ($Qnewproducts->numberOfRows()) {
        ...
        while ($Qnewproducts->next()) {
            ...
            $data[] = array(
            	'products_id' => $Qnewproducts->value('products_id'),
                'products_name' => $Qnewproducts->value('products_name'),
                'products_price' => $product->getPriceFormated(true),
                'is_specials' => ($Qnewproducts->value('specials_price') == NULL) ? FALSE : TRUE,
                'is_featured' => ($Qnewproducts->value('featured_products_id') == NULL) ? FALSE : TRUE,
                'products_image' => $osC_Image->show($Qnewproducts->value('image'), $Qnewproducts->value('products_name')));
...
function get_feature_products() {
    ...
    if ($current_category_id < 1) {
        ...
        select p.products_id, p.products_tax_class_id, p.products_price, pd.products_name, pd.products_keyword, pf.sort_order, i.image, s.specials_new_products_price as specials_price from :table_products p inner join :table_products_to_stores p2s on (p2s.products_id = p.products_id and p2s.stores_id = :stores_id) inner join :table_products_description pd on (p.products_id = pd.products_id and pd.language_id = :language_id) left join :table_products_images i on (p.products_id = i.products_id and i.default_flag = :default_flag) inner join :table_products_frontpage pf on (p.products_id = pf.products_id) left join :table_specials s on (p.products_id = s.products_id and s.status = 1 and s.start_date <= now() and s.expires_date >= now()) where p.products_status = 1 order by pf.sort_order limit :max_display_feature_products');
    } else {
        $Qfeatureproducts = $osC_Database->query('select distinct p.products_id, p.products_tax_class_id, p.products_price, pd.products_name, pf.sort_order, pd.products_keyword, i.image, s.specials_new_products_price as specials_price from :table_products p inner join :table_products_to_stores p2s on (p2s.products_id = p.products_id and p2s.stores_id = :stores_id) inner join :table_products_description pd on (p.products_id = pd.products_id and pd.language_id = :language_id) left join :table_products_images i on (p.products_id = i.products_id and i.default_flag = :default_flag) inner join :table_products_frontpage pf on (p.products_id = pf.products_id) left join :table_specials s on (p.products_id = s.products_id and s.status = 1 and s.start_date <= now() and s.expires_date >= now())left join :table_products_to_categories p2c on (p2c.products_id = p.products_id) inner join :table_categories c on (c.parent_id = :parent_id and c.categories_id = p2c.categories_id) where p.products_status = 1 order by pf.sort_order limit :max_display_feature_products');
    ...
    $Qfeatureproducts->bindTable(':table_products_to_stores', TABLE_PRODUCTS_TO_STORES);
    ...
    $Qfeatureproducts->bindInt(':stores_id', STORE_ID);
