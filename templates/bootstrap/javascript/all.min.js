...
var TocAutoCompleter=new Class({Extends:Autocompleter.Request.JSON,options:{remoteUrl:"json.php",sessionName:"sid",sessionId:null,postData:{module:"auto_completer",action:"get_products"},minLength:3,filterSubset:!0,cache:!0,delay:0,imageGroup:"thumbnail",moreBtnText:"Get More"},initialize:function(t,e){this.options.postVar=t,this.parent(t,this.options.remoteUrl,e),this.options.postData[this.options.sessionName]=this.options.sessionId,e.template&&(this.options.postData.template=e.template)},showChoices:function(){"thumbnail"==this.options.imageGroup&&this.choices.addClass("useThumbnail");var t=this.options.choicesMatch,e=this.choices.getFirst(t);if(this.selected=this.selectedValue=null,this.fix){var i,s=this.element.getCoordinates(this.relative),o=this.element.getSize();this.options.width&&this.choices.setStyle("width",this.options.width+"px"),i=this.choices.getSize(),this.relative||(this.relative=i.x-o.x||o.x-i.x),this.choices.setStyles({left:s.left-this.relative,top:s.bottom}),this.choices.addEvent("mouseleave",function(){this.hideChoices()}.bind(this))}if(e){this.visible||(this.visible=!0,this.choices.setStyle("display",""),this.fx&&this.fx.start(1),this.fireEvent("onShow",[this.element,this.choices])),(this.options.selectFirst||this.typeAhead||e.inputValue==this.queryValue)&&this.choiceOver(e,this.typeAhead);var h=this.choices.getChildren(t),n=this.options.maxChoices,a={overflowY:"hidden",height:""};if(this.overflown=!1,h.length>n){var l=h[n-1];a.overflowY="scroll",a.height=l.getCoordinates(this.choices).bottom,this.overflown=!0}if(this.choices.setStyles(a),this.fix.show(),this.options.visibleChoices){var c=document.getScroll(),r=document.getSize(),p=this.choices.getCoordinates();p.right>c.x+r.x&&(c.x=p.right-r.x),p.bottom>c.y+r.y&&(c.y=p.bottom-r.y),window.scrollTo(Math.min(c.x,p.left),Math.min(c.y,p.top))}if(null===this.choices.getElement("div.more")){var u=new Element("div",{"class":"more"}),m=new Element("a",{href:"#","class":"button squre medium btn",html:this.options.moreBtnText});this.choices.adopt(u),u.adopt(m),m.addEvent("click",function(t){return t.stop(),this.element.getParent("form").submit(),!1}.bind(this))}}},update:function(t){this.choices.empty(),this.cached=t;var e=t&&$type(t);!e||"array"==e&&!t.length||"hash"==e&&!t.getLength()?(this.options.emptyChoices||this.hideChoices).call(this):(this.options.maxChoices<t.length&&!this.options.overflow&&(t.length=this.options.maxChoices),t.each(this.options.injectChoice||function(t){var e=new Element("li",{html:t,"class":"clearfix"}),i=e.getElement("a"),s=i.get("text"),o=this.markQueryValue(s);e.inputValue=s,i.set("html",o),this.addChoiceEvents(e).inject(this.choices)},this),this.showChoices())},markQueryValue:function(t){var e=this.options.markQuery&&this.queryValue?t.replace(new RegExp("("+(this.options.filterSubset?"":"^")+this.queryValue.escapeRegExp()+")",this.options.filterCase?"":"i"),'<span class="autocompleter-queried">$1</span>'):t;return e},query:function(){this.element.removeClass("auto-loaded").addClass("auto-loading"),this.parent()},queryResponse:function(t){this.parent(t),this.element.removeClass("auto-loading").addClass("auto-loaded")},choiceSelect:function(t){var e=t.getElement("a");this.parent(t),window.location=e.getProperty("href")}});
var PopupCart=new Class({Implements:[Options],options:{remoteUrl:"json.php",sessionName:"sid",sessionId:null,isCartExpanded:!1,triggerEl:$("popupCart"),container:$("pageHeader"),itemsEl:$("popupCartItems"),clsCollapsed:"cartCallpased",clsExpanded:"cartExpanded",clsCartText:"cartText",contentEl:$("pageContent")||$("content-center"),enableDelete:"yes",enableFlyEffect:!0,clsAddBtn:".ajaxAddToCart",clsImage:".productImage",dlgConfirmStatus:!0},initialize:function(e){this.setOptions(e),this.registerEvents()},registerEvents:function(){this.options.triggerEl.addEvents({mouseover:function(e){e.stop(),0==this.options.isCartExpanded&&(this.getShoppingCart(),$$("."+this.options.clsCartText).each(function(e){e.hasClass(this.options.clsCollapsed)&&e.removeClass(this.options.clsCollapsed),e.addClass(this.options.clsExpanded)}.bind(this)))}.bind(this)}),"no"===this.options.enableDelete&&(this.options.enableFlyEffect=!1),1==this.options.enableFlyEffect&&this.enableFlyingEffects()},getShoppingCart:function(){var e={action:"get_cart_contents",enable_delete:this.options.enableDelete};this.sendRequest(e,function(e){this.displayCart(e)}.bind(this))},displayCart:function(e){var t,i,s,n=JSON.decode(e),a=this.options.triggerEl.getCoordinates();1==n.success&&($defined(this.cartContainer)?this.cartContainer.set("html",n.content):(this.cartContainer=new Element("div",{html:n.content,id:"popupCartContent",styles:{position:"absolute",opacity:0}}),this.cartContainer.addEvent("mouseleave",function(e){e.stop(),i.start(1,0),this.options.isCartExpanded=!1,$$("."+this.options.clsCartText).each(function(e){e.hasClass(this.options.clsExpanded)&&e.removeClass(this.options.clsExpanded),e.addClass(this.options.clsCollapsed)}.bind(this))}.bind(this))),this.options.container.adopt(this.cartContainer),t=this.cartContainer.getCoordinates(),this.cartContainer.setStyles({left:a.left-(t.width-a.width),top:a.top+a.height/5}),i=new Fx.Tween(this.cartContainer,{duration:100,link:"cancel",property:"opacity"}),i.start(0,1),"yes"==this.options.enableDelete&&(s=this.cartContainer.getElements(".removeBtn"),s.length>0&&s.each(function(e){e.addEvent("click",function(t){t.stop();var i=e.get("data-pid");return this.removeProduct(i,e),!1}.bind(this))},this),this.options.itemsEl.set("text",n.total)),this.options.isCartExpanded=!0)},enableFlyingEffects:function(){this.options.contentEl&&this.options.contentEl.addEvent("click:relay("+this.options.clsAddBtn+")",function(e){var t=e.target;if(e.stop(),t.hasClass("disabled"))return!1;t.set("disabled","disabled");var i=[],s=t.get("id"),n="";if(s.test("^ac_[a-z]+_[0-9]+$","i")){var a=s.split("_").getLast(),o=null,d=$$(".variantCombobox select"),l=$$(".variants_"+a+" select");d.length>0?o=d:l.length>0&&(o=l),null!==o&&o.each(function(e){var t=e.id.toString(),i=t.substring(9,t.indexOf("]"));n+=i+":"+e.value+";"}.bind(this))}else if(s.test("^ac_[a-z]+_[0-9]+(?:#(?:[0-9]+:?[0-9]+)+(?:;?(?:[0-9]+:?[0-9]+)+)*)*$","i"))var r=s.split("_").getLast(),p=r.split("#"),a=p[0],n=p[1];var c={action:"add_product",pID:a};return $defined($("quantity"))&&(c.pQty=$("quantity").get("value")),null!=$("qty_"+a)&&(c.pQty=$("qty_"+a).get("value")),n&&(c.variants=n),$defined($("senders_name"))&&""!=$("senders_name").value?c.senders_name=$("senders_name").value:$defined($("senders_name"))&&""==$("senders_name").value&&i.push(this.options.error_sender_name_empty),$defined($("senders_email"))&&""!=$("senders_email").value?c.senders_email=$("senders_email").value:$defined($("senders_email"))&&""==$("senders_email").value&&i.push(this.options.error_sender_email_empty),$defined($("recipients_name"))&&""!=$("recipients_name").value?c.recipients_name=$("recipients_name").value:$defined($("recipients_name"))&&""==$("recipients_name").value&&i.push(this.options.error_recipient_name_empty),$defined($("recipients_email"))&&""!=$("recipients_email").value?c.recipients_email=$("recipients_email").value:$defined($("recipients_email"))&&""==$("recipients_email").value&&i.push(this.options.error_recipient_email_empty),$defined($("message"))&&""!=$("message").value?c.message=$("message").value:$defined($("message"))&&""==$("message").value&&i.push(this.options.error_message_empty),$defined($("gift_certificate_amount"))&&""!=$("gift_certificate_amount").value?c.gift_certificate_amount=$("gift_certificate_amount").value:$defined($("gift_certificate_amount"))&&""==$("gift_certificate_amount").value&&i.push(this.options.error_message_open_gift_certificate_amount),i.length>0?(alert(i.join("\n")),t.erase("disabled"),!1):(this.sendRequest(c,function(e){var i=JSON.decode(e);1==i.success?(this.doFlyingEffects(t,i.items),1==this.options.dlgConfirmStatus&&this.showConfirmation(i.confirm_dialog)):t.erase("disabled")}.bind(this)),!1)}.bind(this))},doFlyingEffects:function(e,t){var i=$(document.body),s=e.closest(this.options.clsImage),n=s.getCoordinates(),a=s.clone().setStyles({position:"absolute",width:n.width,height:n.height,left:n.left,top:n.top,"z-index":9999}),o=this.options.triggerEl.getCoordinates();i.adopt(a),a.set("morph",{duration:300,onComplete:function(){a.fade("out"),function(){a.destroy()}.delay(500),e.erase("disabled"),this.options.itemsEl.set("text",t)}.bind(this)}).morph({width:n.width/4,height:n.height/4,top:o.top,left:o.left})},removeProduct:function(e,t){var i,s=t.getParent("tr"),n=t.getParent("table").getNext("table"),a={action:"remove_product",pID:e};this.sendRequest(a,function(e){i=JSON.decode(e),i.success&&(s.addClass("animated slideOutUp"),function(){s.destroy()}.delay(300),this.options.itemsEl.set("text",i.total),n.set("html",i.order_totals))}.bind(this))},showConfirmation:function(e){null!=e&&("undefined"!=typeof this.dlg?this.dlg.update(e):this.dlg=new popDialog(e),this.dlg.show(),null!=$("btnContinue")&&$("btnContinue").addEvent("click",function(e){return e.stop(),this.dlg.doAnimate("slideOutUp"),this.dlg.hide(),!1}.bind(this)))},sendRequest:function(e,t){var i={template:this.options.template,module:"popup_cart",method:"post"};i[this.options.sessionName]=this.options.sessionId,$extend(i,e);new Request({url:this.options.remoteUrl,data:i,onSuccess:t.bind(this)}).send()}});
var TocBookmark=new Class({Implements:[Options],options:{title:document.title,url:window.location.href},initialize:function(t){this.setOptions(t);var o=$(this.options.bookmark),i=new Element("a"),e=new Element("img",{title:this.options.text,src:this.options.img});window.opera&&window.print?i.setProperties({href:this.options.url,title:this.options.title,rel:"sidebar"}):i.setStyle("cursor","pointer"),i.appendChild(e),i.appendText(this.options.text),o.appendChild(i),i.addEvent("click",function(){this.onBookmarkClicked()}.bind(this))},onBookmarkClicked:function(){window.sidebar?window.sidebar.addPanel(this.options.title,this.options.url,""):document.all&&window.external.AddFavorite(this.options.url,this.options.title),window.opera&&window.print?alert("Press ctrl+D to bookmark (Command+D for macs) after you click Ok"):window.chrome&&alert("Press ctrl+D to bookmark (Command+D for macs) after you click Ok")}});
