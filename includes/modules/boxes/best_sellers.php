  ...
  class osC_Boxes_best_sellers extends osC_Modules {
    ...
    function initialize() {
      ...
      if (isset($current_category_id) && ($current_category_id > 0)) {
        $Qbestsellers = $osC_Database->query('select distinct p.products_id, pd.products_name, pd.products_keyword from :table_products p, :table_products_description pd, :table_products_to_categories p2c, :table_categories c, :table_products_to_stores p2s where p.products_status = 1 and p.products_ordered > 0 and p.products_id = p2s.products_id and p2s.stores_id = :stores_id and p.products_id = pd.products_id and pd.language_id = :language_id and p.products_id = p2c.products_id and p2c.categories_id = c.categories_id and :current_category_id in (c.categories_id, c.parent_id) order by p.products_ordered desc, pd.products_name limit :max_display_bestsellers');
        ...
        $Qbestsellers->bindTable(':table_products_to_stores', TABLE_PRODUCTS_TO_STORES);
        $Qbestsellers->bindTable(':table_categories', TABLE_CATEGORIES);
        $Qbestsellers->bindInt(':language_id', $osC_Language->getID());
        $Qbestsellers->bindInt(':stores_id', STORE_ID);
        ...
        if (BOX_BEST_SELLERS_CACHE > 0) {
          $Qbestsellers->setCache('box_best_sellers-' . STORE_ID . '-' . $current_category_id . '-' . $osC_Language->getCode(), BOX_BEST_SELLERS_CACHE);
        }

        $Qbestsellers->execute();
      } else {
        $Qbestsellers = $osC_Database->query('select p.products_id, pd.products_name, pd.products_keyword from :table_products p, :table_products_description pd, :table_products_to_stores p2s where p.products_status = 1 and p.products_ordered > 0 and p.products_id = p2s.products_id and p2s.stores_id = :stores_id and p.products_id = pd.products_id and pd.language_id = :language_id order by p.products_ordered desc, pd.products_name limit :max_display_bestsellers');
        $Qbestsellers->bindTable(':table_products', TABLE_PRODUCTS);
        $Qbestsellers->bindTable(':table_products_description', TABLE_PRODUCTS_DESCRIPTION);
        $Qbestsellers->bindTable(':table_products_to_stores', TABLE_PRODUCTS_TO_STORES);
        $Qbestsellers->bindInt(':language_id', $osC_Language->getID());
        ...
        if (BOX_BEST_SELLERS_CACHE > 0) {
          $Qbestsellers->setCache('box_best_sellers-0-' . STORE_ID . '-' . $osC_Language->getCode(), BOX_BEST_SELLERS_CACHE);
        }
